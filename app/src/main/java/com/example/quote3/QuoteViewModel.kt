...
class QuoteViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // Reference to DatabaseHelper for accessing the SQLite database
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                withContext(Dispatchers.Main) {
                    quote.value = randomQuote ?: "No inspirational quotes available."
                }
            } catch (e: Exception) {
                // Handle any exceptions gracefully
                withContext(Dispatchers.Main) {
                    quote.value = "Error fetching quote. Please try again."
                }
            }
        }
    }
